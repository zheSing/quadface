...
vcg::GlTrimesh<PolyMesh> glWrapQuad;
vcg::GLField<FieldTriMesh> glField;
Quadrangulation<FieldTriMesh>::QuadPara QuadP;
...
enum gui_mode{ None, VertexUI, EdgeUI, SymmUI, QuadUI } uimode, uimode_;
enum draw_mode{ TextureWire, VertColorWire, Texture, VertColor } currentDraw;
...
void TW_CALL Quadrangulate(void*)
{
    if (!fielded)
        DoBatchProcess();
    QuadP.Alpha = AlphaSing;
    QuadP.ScaleFact = ScaleFact;
    Quadrangulation<FieldTriMesh>::QuadrangulateFromTri(tri_mesh, QuadP, quad_mesh);
    uimode_ = QuadUI;
}
...
void InitFieldBar(QWidget *w)
{
    if (uimode == None)
    {
        ...
        TwEnumVal colormodes[4] = { {TextureWire, "TextureWire"}, {Texture, "Texture"},
                                    {VertColorWire, "VertColorWire"}, {VertColor, "VertColor"}};
        // Create a type for the enum shapeEV
        TwType colorMode = TwDefineEnum("DrawMode", colormodes, 4);
        TwAddVarRW(barQuad, "Draw Mode", colorMode, &currentDraw, " keyIncr='<' keyDecr='>' help='Change color mode.' ");
        ...
        TwAddSeparator(barQuad, "sep6", "label=''");
        TwAddVarRW(barQuad, "alpha_quad", TW_TYPE_DOUBLE, &AlphaSing, "label='AlphaSingular'");
        TwAddVarRW(barQuad, "ScaleFactor", TW_TYPE_DOUBLE, &ScaleFact, "label='ScaleFactor'");
        TwAddVarRW(barQuad, "minFactor", TW_TYPE_DOUBLE, &minFactor, "label='minFactor'");
        TwAddVarRW(barQuad, "maxFactor", TW_TYPE_DOUBLE, &maxFactor, "label='maxFactor'");
        TwAddButton(barQuad, "Quadrangulate", Quadrangulate, 0, "label='Quadrangulate'");
    ...
    else if (uimode == SymmUI)
    {
        TwDeleteAllBars();
        barQuad = TwNewBar("Symmetry EdgeUI Selection");
        TwAddVarRW(barQuad, "SymmSmooth", TW_TYPE_INT32, &SymmSmooth, "label='SymmSmooth");
        TwAddButton(barQuad, "SymmConfirm", SymmConfirm, 0, "label='Generate'");
    }

    else if (uimode == QuadUI)
    {
        TwDeleteAllBars();
    }
}
    ...
    : QOpenGLWidget(parent)
{
    ...
    if (has_symm)
    {
        std::cout<<"*** Loading Symmetry Edge ***"<<std::endl;
        bool HasRead=tri_mesh.LoadSymmetry(pathSymm);
        assert(HasRead);
    }
    tri_mesh.InitEdgeType();
...
void GLWidget::paintGL ()
{
    ...
    if(uimode != QuadUI && tri_mesh.vert.size()>0)
    {
        ...
        // drawing
        if (currentDraw == TextureWire)
            glWrap.Draw(vcg::GLW::DMFlatWire,vcg::GLW::CMPerFace,vcg::GLW::TMPerWedge);
        else if (currentDraw == Texture)
            glWrap.Draw(vcg::GLW::DMFlat,vcg::GLW::CMPerFace,vcg::GLW::TMPerWedge);
        else if (currentDraw == VertColorWire)
            glWrap.Draw(vcg::GLW::DMFlatWire,vcg::GLW::CMPerVert,vcg::GLW::TMNone);
        else if (currentDraw == VertColor)
            glWrap.Draw(vcg::GLW::DMFlat,vcg::GLW::CMPerVert,vcg::GLW::TMNone);
        // draw field
        if (drawfield)
        {
    ...
    else if (uimode == QuadUI && quad_mesh.vert.size() > 0)
    {
        vcg::glScale(2.0f/quad_mesh.bbox.Diag());
        glTranslate(-quad_mesh.bbox.Center());
        if (has_texture)
        {
            glEnable(GL_TEXTURE_2D);
            glBindTexture(GL_TEXTURE_2D, tID);
        ...
        // drawing
        if (currentDraw == TextureWire)
            glWrapQuad.Draw(vcg::GLW::DMFlatWire,vcg::GLW::CMPerFace,vcg::GLW::TMPerWedge);
        else if (currentDraw == Texture)
            glWrapQuad.Draw(vcg::GLW::DMFlat,vcg::GLW::CMPerFace,vcg::GLW::TMPerWedge);
        else if (currentDraw == VertColorWire)
            glWrapQuad.Draw(vcg::GLW::DMFlatWire,vcg::GLW::CMPerVert,vcg::GLW::TMNone);
        else if (currentDraw == VertColor)
            glWrapQuad.Draw(vcg::GLW::DMFlat,vcg::GLW::CMPerVert,vcg::GLW::TMNone);
    }
    ...
    if(!TwMousePressQt(this,e))
    {
        ...
        if (uimode != None && e->button() == Qt::RightButton)
        {
            ...
            else if (uimode == SymmUI)
            {
                ...
                if (bvh_tree.Intersect(ray, inter))
                {
                    size_t cnt = SymmetryManager<FieldTriMesh>::TraceAxisByDirection(tri_mesh, inter, SymmSmooth);
